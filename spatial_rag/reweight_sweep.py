...
def _export_variant_db(
    *,
    ...
    base_object_arr_dinov3: Optional[np.ndarray],
    scored_rows: Sequence[Mapping[str, Any]],
    summary: Mapping[str, Any],
) -> Dict[str, Any]:
    ...
    object_dinov3_rows: List[np.ndarray] = []
    for row in kept_object_rows:
        source_row = row.get("dinov3_embedding_row_index")
        try:
            source_row_int = int(source_row) if source_row not in (None, "") else None
        except Exception:
            source_row_int = None
        if (
            base_object_arr_dinov3 is not None
            and base_object_arr_dinov3.ndim == 2
            and source_row_int is not None
            and 0 <= int(source_row_int) < int(base_object_arr_dinov3.shape[0])
        ):
            row["dinov3_embedding_row_index"] = int(len(object_dinov3_rows))
            object_dinov3_rows.append(np.asarray(base_object_arr_dinov3[int(source_row_int)], dtype=np.float32))
        else:
            row["dinov3_embedding_row_index"] = None
    if object_dinov3_rows:
        object_arr_dinov3 = np.vstack(object_dinov3_rows).astype(np.float32)
    else:
        dino_dim = int(base_object_arr_dinov3.shape[1]) if base_object_arr_dinov3 is not None and base_object_arr_dinov3.ndim == 2 else 0
        object_arr_dinov3 = np.zeros((0, dino_dim), dtype=np.float32)
    ...
    np.save(output_root / "object_dinov3_emb.npy", np.asarray(object_arr_dinov3, dtype=np.float32))
    ...
    report = _rebuild_build_report(
        ...
        object_arr_dinov3=np.asarray(object_arr_dinov3, dtype=np.float32),
        per_entry_threshold_stats=per_entry_threshold_stats,
        summary=summary,
    )
...
def run_reweight_sweep(
    ...
    *,
    ...
    object_arr_dinov3 = None
    object_dinov3_path = base_root / "object_dinov3_emb.npy"
    if object_dinov3_path.exists():
        object_arr_dinov3 = np.load(object_dinov3_path).astype("float32")
    ...
    for summary, scored_rows in export_queue:
        ...
        exported_summary = _export_variant_db(
            ...
            base_object_arr_dinov3=object_arr_dinov3,
            scored_rows=scored_rows,
            summary=summary,
        )
