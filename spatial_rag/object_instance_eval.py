...
def compute_pairwise_cosines(
    ...
    dinov3_emb: Optional[np.ndarray] = None,
    ...
    for pair in pair_gt:
        ...
        if dinov3_emb is not None and max_required >= int(dinov3_emb.shape[0]):
            raise KeyError(f"Pair {pair.pair_id} references object id {max_required} outside dinov3 embedding range")
        a = int(pair.obj_a_id)
        b = int(pair.obj_b_id)
        results.append(
            PairScoreRecord(
                ...
                dinov3_cosine=(
                    float(np.dot(dinov3_emb[a], dinov3_emb[b]))
...
def summarize_similarity_metrics(pair_scores: Sequence[PairScoreRecord]) -> Dict[str, Any]:
    ...
    if any(math.isfinite(float(getattr(row, "dinov3_cosine", float("nan")))) for row in pair_scores):
        summary["dinov3"] = _representation_metrics(pair_scores, field_name="dinov3_cosine")
    return summary
...
def _write_summary_markdown(path: Path, report: Mapping[str, Any]) -> None:
    ...
    for key in ("short", "long", "graph", "dinov3"):
...
def _load_representation_embeddings(
    ...
    if "dinov3" in modes:
        loaded = load_object_dinov3_db(db_dir)
        if loaded is None:
            raise FileNotFoundError(f"Missing DINOv3 object DB artifacts in {db_dir}")
        ...
        if meta_rows:
            ...
            embeddings["dinov3"] = _l2_normalize_rows(dense)
    return embeddings
...
def _prepare_pair_scores(
    ...
    pair_scores = compute_pairwise_cosines(
        ...
        long_emb=long_emb,
        graph_emb=graph_emb,
        dinov3_emb=embeddings.get("dinov3"),
    )
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--text_modes", type=str, default="short,long,graph", help="Comma-separated list of representations to evaluate, e.g. short,long,graph,dinov3")
