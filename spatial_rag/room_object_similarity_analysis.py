...
def _load_representation_embeddings(
    ...
    dino_loaded = load_object_dinov3_db(db_dir)
...
def _parse_fused_text_modes(value: Any) -> List[str]:
    ...
    for item in raw.split(","):
        ...
        if not mode:
            continue
        if mode not in {"short", "long", "dinov3"}:
...
def analyze_rooms(
    ...
    for folder_name, room, room_kind, selected_rows in selected_rooms:
        ...
        matrices = {
            ...
            for mode in ("short", "long", "graph", "dinov3")
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=str,
        default="short,long",
        help="Comma-separated fused modes to generate: short, long, dinov3, or combinations like short,long,dinov3",
    )
